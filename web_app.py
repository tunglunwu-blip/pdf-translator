...
def split_block_into_columns(lines: Sequence[dict]) -> List[List[dict]]:
    """核心防重疊演算：依據 X 座標將被誤併的雙欄文字強制切開"""
    if not lines:
        return []
    ...
    for line in lines:
        ...
        matched_col = None
        for col in columns:
            avg_x0 = sum(l["bbox"][0] for l in col) / len(col)
            if abs(x0 - avg_x0) < 35.0:  # X 座標相近者視為同一欄
                matched_col = col
                break
        if matched_col is not None:
            matched_col.append(line)
        else:
            columns.append([line])
            
    for col in columns:
        col.sort(key=lambda l: l["bbox"][1])  # 保持垂直順序
    return columns
...
def extract_regions(page: fitz.Page, page_number: int) -> List[TextRegion]:
    ...
    for block in page_dict.get("blocks", []):
        ...
        # 自動防雙欄混淆
        split_columns = split_block_into_columns(raw_lines)

        for col_lines in split_columns:
            line_texts = []
            spans = []
            directions = []
            rect = None
            ...
            if not spans or rect is None or rect.is_empty:
                continue
            ...
            regions.append(
                TextRegion(
                    ...
                    rotation=line_rotation(directions[0] if directions else (1.0, 0.0)),
...
def insert_fitted_text(page: fitz.Page, region: TextRegion, font_name: str, font_path: Optional[Path], font_scale: float, min_font_size: float) -> bool:
    rect = fitz.Rect(region.rect)
    size = max(min_font_size, region.font_size * font_scale)

    # 關鍵：改回 render_mode = 0，徹底關閉描邊，字體不再變粗黑塊
    render_mode = 0
...
st.markdown("##### 免費 · 雙欄完美排版 · 高清清晰字體")

with st.sidebar:
    st.header("⚙️ 翻譯設定")
    workers = st.slider("併行工作數 (Workers)", min_value=1, max_value=5, value=3)
    delay = st.slider("請求延遲 (秒)", min_value=0.0, max_value=2.0, value=0.20, step=0.05)
    font_scale = st.slider("中文字縮放比", min_value=0.5, max_value=1.0, value=0.82) # 預設改為 0.82 更適合雙欄
...
if uploaded_file is not None:
    ...
    if st.button("🚀 開始翻譯全書", type="primary"):
        ...
        try:
            status_box.info("🔍 正在分析 PDF 雙欄結構與頁面...")
            ...
            status_box.info(f"📄 全書共 {total_pages} 頁，找到 {len(all_regions)} 個區域。開始連線翻譯...")
            ...
            status_box.info("🎨 翻譯完成，正在精準繪製高清中文排版...")
            ...
            for page_index in range(total_pages):
                ...
                if regions:
                    ...
                    for region in regions:
                        insert_fitted_text(page, region, font_name, chinese_font, font_scale, min_font_size)
